...
async function validateDocument(textDocument) {
    ...
    while ((match = classDecl.exec(text)) !== null)
        declaredFuncs.add(match[1]);
    while ((match = forVar.exec(text)) !== null)
        declaredVars.add(match[1]);
    ...
    for (let lineNum = 0; lineNum < lines.length; lineNum++) {
        ...
        if (lineInBC || trimmed.startsWith('/*') || trimmed.startsWith('*'))
            continue;
...
connection.onCompletion((_params) => {
    ...
    let m;
    while ((m = varDecl2.exec(text)) !== null)
        items.push({ label: m[1], kind: node_1.CompletionItemKind.Variable });
    while ((m = conDecl2.exec(text)) !== null)
        items.push({ label: m[1], kind: node_1.CompletionItemKind.Constant });
    while ((m = funcDecl2.exec(text)) !== null)
        items.push({ label: m[1], kind: node_1.CompletionItemKind.Function });
    while ((m = classDecl2.exec(text)) !== null)
        items.push({ label: m[1], kind: node_1.CompletionItemKind.Class });
    while ((m = forVar2.exec(text)) !== null)
        items.push({ label: m[1], kind: node_1.CompletionItemKind.Variable });
