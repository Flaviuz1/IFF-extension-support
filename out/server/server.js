...
connection.onCompletion((params) => {
    ...
    const items = [];
    for (const kw of KEYWORDS) items.push({ label: kw, kind: node_1.CompletionItemKind.Keyword  });
    for (const b  of BUILTINS)  items.push({ label: b,  kind: node_1.CompletionItemKind.Constant });
    ...
    let m;
    while ((m = varDecl2.exec(text))   !== null) items.push({ label: m[1], kind: node_1.CompletionItemKind.Variable });
    while ((m = conDecl2.exec(text))   !== null) items.push({ label: m[1], kind: node_1.CompletionItemKind.Constant });
    while ((m = funcDecl2.exec(text))  !== null) items.push({ label: m[1], kind: node_1.CompletionItemKind.Function });
    while ((m = classDecl2.exec(text)) !== null) items.push({ label: m[1], kind: node_1.CompletionItemKind.Class    });
    while ((m = forVar2.exec(text))    !== null) items.push({ label: m[1], kind: node_1.CompletionItemKind.Variable });
